...
data "huaweicloud_compute_flavors" "test" {
  count = var.ecs_instance_flavor_id == "" ? 1 : 0

  performance_type = var.ecs_instance_flavor_performance_type
  cpu_core_count   = var.ecs_instance_flavor_cpu_core_count
  memory_size      = var.ecs_instance_flavor_memory_size
}

data "huaweicloud_images_images" "test" {
  count = var.ecs_instance_image_id == "" ? 1 : 0

  flavor_id  = var.ecs_instance_flavor_id == "" ? try(data.huaweicloud_compute_flavors.test[0].flavors[0].id, null) : var.ecs_instance_flavor_id
  visibility = var.ecs_instance_image_visibility
  os         = var.ecs_instance_image_os_type
}

resource "huaweicloud_compute_instance" "test" {
  name               = var.ecs_instance_name
  flavor_id          = var.ecs_instance_flavor_id == "" ? try(data.huaweicloud_compute_flavors.test[0].flavors[0].id, null) : var.ecs_instance_flavor_id
  image_id           = var.ecs_instance_image_id == "" ? try(data.huaweicloud_images_images.test[0].images[0].id, null) : var.ecs_instance_image_id
...
resource "huaweicloud_apig_instance" "test" {
  ...
  available_zones = [
    data.huaweicloud_availability_zones.test[0].names[0],
  ]
}
...
resource "huaweicloud_apig_custom_authorizer" "test" {
  ...
  function_version = var.custom_authorizer_function_version
...
resource "huaweicloud_apig_channel" "test" {
  ...
  port             = var.channel_port
  balance_strategy = var.channel_balance_strategy
  member_type      = "ecs"
  type             = "builtin"
...
resource "huaweicloud_apig_api" "test" {
  ...
  web {
    ...
    path             = var.api_web_path
    vpc_channel_id   = huaweicloud_apig_channel.test.id
    request_method   = var.api_web_request_method
    request_protocol = var.api_web_request_protocol
    timeout          = var.api_web_timeout
  }
}
