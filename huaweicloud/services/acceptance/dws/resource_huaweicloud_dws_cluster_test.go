...
// Due to cluster quota restrictions, this acceptance test uses serial method.
func TestAccResourceCluster_basicV1(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccDwsCluster_basic_step2(name, dws.PublicBindTypeAuto, updatePassword),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(resourceName, "name", name),
			...
			{
				Config: testAccDwsCluster_basic_step3(name, dws.PublicBindTypeAuto, updatePassword),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(resourceName, "name", name),
...
func testAccDwsCluster_basic_step2(rName, publicIpBindType, password string) string {
	return fmt.Sprintf(`
%[1]s
...
`, common.TestBaseNetwork(rName), rName, password, acceptance.HW_ENTERPRISE_MIGRATE_PROJECT_ID_TEST, publicIpBindType)
}

func testAccDwsCluster_basic_step3(rName, publicIpBindType, password string) string {
	return fmt.Sprintf(`
%[1]s
...
func TestAccResourceCluster_basicV2(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Assert that all ELBs are unbound and delete all tags.
			{
...
`, testAccDwsCluster_basicV2_base(rName), rName, password, acceptance.HW_ENTERPRISE_PROJECT_ID_TEST, dws.PublicBindTypeNotUse)
}
...
`, testAccDwsCluster_basicV2_base(rName), rName, password, acceptance.HW_ENTERPRISE_MIGRATE_PROJECT_ID_TEST, dws.PublicBindTypeNotUse)
}
...
`, baseNetwork, rName, acceptance.HW_DWS_MUTIL_AZS, password, dws.PublicBindTypeAuto)
}
