...
func TestAccDataSourceLogicalClusters_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			...
			acceptance.TestAccPreCheckDwsLogicalClusterId(t)
...
output "is_existed_logical_cluster" {
  value = contains(local.logical_cluster_ids, "%[2]s") && contains(local.logical_cluster_names, "%[3]s")
}
`, acceptance.HW_DWS_LOGICAL_MODE_CLUSTER_ID, acceptance.HW_DWS_LOGICAL_CLUSTER_ID, acceptance.HW_DWS_LOGICAL_CLUSTER_NAME)
}
